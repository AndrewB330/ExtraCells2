...
public class ItemBlockCertusTank extends ItemBlock implements IFluidContainerItem {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        if (stack != null && stack.hasTagCompound()) {
            if (FluidStack.loadFluidStackFromNBT(stack.getTagCompound().getCompoundTag("tileEntity")) != null)
                list.add(FluidStack.loadFluidStackFromNBT(
    ...
    public int fill(ItemStack container, FluidStack resource, boolean doFill) {
        ...
        if (!doFill) {
            if (container.stackTagCompound == null || !container.stackTagCompound.hasKey("tileEntity")) {
                return Math.min(TileEntityCertusTank.CAPACITY, resource.amount);
            }
            ...
            return Math.min(TileEntityCertusTank.CAPACITY - stack.amount, resource.amount);
        }
        ...
        if (!container.stackTagCompound.hasKey("tileEntity")
            ...
            if (TileEntityCertusTank.CAPACITY < resource.amount) {
                fluidTag.setInteger("Amount", TileEntityCertusTank.CAPACITY);
                container.stackTagCompound.setTag("tileEntity", fluidTag);
                return TileEntityCertusTank.CAPACITY;
            }
        ...
        int filled = TileEntityCertusTank.CAPACITY - stack.amount;
        if (resource.amount < filled) {
            stack.amount += resource.amount;
            filled = resource.amount;
        } else {
            stack.amount = TileEntityCertusTank.CAPACITY;
        }
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player,
        ...
        if (stack != null && stack.hasTagCompound()) {
            NBTTagCompound tankNbt = stack.getTagCompound().getCompoundTag("tileEntity");
            ((TileEntityCertusTank) world.getTileEntity(x, y, z)).getTank().readFromNBT(tankNbt);
        }
