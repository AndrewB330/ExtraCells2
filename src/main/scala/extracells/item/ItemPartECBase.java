...
public class ItemPartECBase extends Item implements IPartItem, IItemGroup {
    ...
    public ItemPartECBase() {
        ...
        for (PartEnum part : PartEnum.values()) {
            for (Map.Entry<Upgrades, Integer> entry : part.getUpgradesMaxLimit().ToAeMap().entrySet()) {
                entry.getKey().registerItem(new ItemStack(this, 1, part.ordinal()), entry.getValue());
            }
        }
    }
    ...
    public EnumRarity getRarity(ItemStack itemStack) {
        if (itemStack != null
                && itemStack.getItemDamage() == PartEnum.ORE_DICT_EXPORT_BUS
                .ordinal())
