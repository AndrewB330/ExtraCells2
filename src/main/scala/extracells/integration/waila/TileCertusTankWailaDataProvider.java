...
public class TileCertusTankWailaDataProvider implements IWailaDataProvider {
    ...
    public NBTTagCompound getNBTData(EntityPlayerMP player, TileEntity tile,
        ...
        if (tile instanceof TileEntityCertusTank) {
            if (((TileEntityCertusTank) tile).getTank().getFluid() == null)
                tag.setInteger("fluidID", -1);
            else {
                tag.setInteger("fluidID", ((TileEntityCertusTank) tile).getTank().getFluid().getFluidID());
                tag.setInteger("currentFluid", ((TileEntityCertusTank) tile).getTank().getFluidAmount());
            }
            tag.setInteger("maxFluid", ((TileEntityCertusTank) tile).getTank().getCapacity());
        }
        return tag;
    }
